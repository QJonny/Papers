...
object Analyze {
  def main(args : Array[String]): Unit = {
    ...
    if (args.length == 0 || args.length > 2) println("You should provide at leath a path and at most a path and an option. Type -h for help.");
...
class Analyzer extends Object with LoadPaper
  ...
  // Analyze a paper
  def analyze(paperPos: String, options: List[String]): Unit = {
